...
def check_state_dict(state_dict, state_dict_disk, logger):
    logger.info("----------- Pre-trained Weights ------------")
    update_model_state = {}
    for key, val in state_dict_disk.items():
        if key in state_dict and state_dict[key].shape == val.shape:
            update_model_state[key] = val
        else:
            logger.info(
                "Not loaded weight: %s, %s" % (key, str(state_dict_disk[key].shape))
            )
...
def mask_points_by_range(points, limit_range, padding=0.0, with_height=True):
    dim = 3 if with_height else 2
    mask = np.all(
        points[:, :dim] >= (np.array(limit_range[:dim]) + padding), axis=1
    ) & np.all(
        points[:, :dim] <= (np.array(limit_range[3 : 3 + dim]) - padding), axis=1
    )
    return mask


def get_voxel_centers(
    voxel_coords, downsample_times, voxel_size, point_cloud_range, dim=3
):
    """
    Args:
        voxel_coords: (N, 3)
    ...
    voxel_size = (
        torch.tensor(voxel_size[:dim], device=voxel_centers.device).float()
        * downsample_times
    )
    pc_range = torch.tensor(
        point_cloud_range[:dim], device=voxel_centers.device
    ).float()
...
class SharedMemoryManager(object):
    ...
    def __init__(self, name, type=0, shape=(1,), dtype=np.float32):
        ...
        if self.type == 0:
            ...
            self.buf = np.ndarray(
                self.shape, dtype=self.dtype, buffer=self.shared_memory.buf
            )
        else:
    ...
    def execute(self, arr=None):
        ...
        if self.type == 0:
            ...
            ret_arr = np.copy(
                np.ndarray(self.shape, dtype=self.dtype, buffer=self.shared_memory.buf)
            )
            return ret_arr
...
def clear_shared_memory(query_name=None):
    if query_name is not None:
        os.system(
            f"kill -9 `ps -ef | grep {query_name} | grep -v grep | awk '{{print $2}}'`"
        )
