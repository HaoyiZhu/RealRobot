...
class NormalizeColorPCD(object):
    def __call__(self, data_dict):
...
class NormalizeCoordPCD(object):
    def __call__(self, data_dict):
...
class ClipGaussianJitterPCD(object):
...
class ChromaticAutoContrastPCD(object):
...
class ChromaticTranslationPCD(object):
...
class ChromaticJitterPCD(object):
...
class RandomColorGrayScalePCD(object):
    def __init__(self, p):
        self.p = p
...
class RandomColorJitterPCD(object):
    """
...
class HueSaturationTranslationPCD(object):
    @staticmethod
    def rgb_to_hsv(rgb):
...
class RandomColorDropPCD(object):
