    ...
    def forward(self, obs_dict):
        ...
        if self.share_rgb_model:
            ...
            for key in self.rgb_keys:
                ...
                if (
                    self.use_depth
                    and key.replace("rgb", "depth") in obs_dict
                    and key.replace("rgb", "depth") in self.depth_keys
                ):
                    depth = obs_dict[key.replace("rgb", "depth")]
                    assert (
                        depth.shape[1:]
                        == self.key_shape_map[key.replace("rgb", "depth")]
                    )
                    if self.only_depth:
                        img = depth
                    else:
