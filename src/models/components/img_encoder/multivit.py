...
class DPTOutputAdapter(nn.Module):
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], input_info: Dict):
        ...
        layers = [self.adapt_tokens(la, input_info) for la in layers]

        # Reshape tokens to spatial representation
        layers = [
            rearrange(la, "b (nh nw) c -> b c nh nw", nh=N_H, nw=N_W) for la in layers
        ]

        # Postprocess activations
        layers = [self.act_postprocess[idx](la) for idx, la in enumerate(layers)]

        # Project layers to chosen feature dim
        layers = [self.scratch.layer_rn[idx](la) for idx, la in enumerate(layers)]
